...
class GATE():
    ...
    def __call__(self, A, X, R, S):
        ...
        features_loss_1 = 1/2 * (tf.reduce_sum(tf.reduce_sum(tf.pow(X - X_, 2))))
    ...
    def cluster_metrics(self, y_true, y_pred):
        y_true = y_true.astype(np.int64)
        y_true = np.argmax(y_true, axis=1)
        acc = accuracy_score(y_true, y_pred)  # not used
    ...
    def __dense_layer(self, Z):
        ...
        logits = tf.layers.dense(inputs=dense3, units=6, activation=None)
        return logits
